...
update() {
  ...
  if [ "$(echo "$INFO" | jq -r '.["Player State"]')" = "Playing" ]; then
    PLAYING=0
    TRACK="$(echo "$INFO" | jq -r .Name | sed 's/\(.\{15\}\).*/\1.../')"
    ARTIST="$(echo "$INFO" | jq -r .Artist | sed 's/\(.\{15\}\).*/\1.../')"
    ALBUM="$(echo "$INFO" | jq -r .Album | sed 's/\(.\{23\}\).*/\1.../')"
    TRACK_SHORT="$(echo "$INFO" | jq -r .Name | sed 's/\(.\{10\}\).*/\1.../')"
    ARTIST_SHORT="$(echo "$INFO" | jq -r .Artist | sed 's/\(.\{10\}\).*/\1.../')"
    ALBUM_SHORT="$(echo "$INFO" | jq -r .Album | sed 's/\(.\{15\}\).*/\1.../')"
    SHUFFLE=$(osascript -e 'tell application "Spotify" to get shuffling')
    REPEAT=$(osascript -e 'tell application "Spotify" to get repeating')
